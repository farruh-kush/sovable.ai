...
def normalize_phone(value: str) -> str:
    ...
    if not compact.startswith("+") or not compact[1:].isdigit() or not 8 <= len(compact[1:]) <= 15:
        raise HTTPException(status_code=422, detail="Use an E.164 phone number such as +14155550123")
    return compact
...
def decode_access_token(settings: AuthSettings, token: str) -> dict[str, Any]:
    try:
        return jwt.decode(token, settings.secret_key, algorithms=ALGORITHMS, audience="sovable-api", issuer="sovable-auth")
...
def provider_config(settings: AuthSettings, provider: str) -> dict[str, Any]:
    if provider == "google":
        return {
            ...
            "redirect_uri": settings.google_redirect_uri or f"{settings.public_base_url}/auth/oauth/google/callback",
    ...
    if provider == "apple":
        return {
            ...
            "client_secret": "",
            "redirect_uri": settings.apple_redirect_uri or f"{settings.public_base_url}/auth/oauth/apple/callback",
