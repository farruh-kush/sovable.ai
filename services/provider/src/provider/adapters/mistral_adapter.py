...
class MistralAdapter(BaseProviderAdapter):
    ...
    def __init__(self, api_key: Optional[str], timeout_seconds: float = 30.0) -> None:
        ...
        self._base_url = _MISTRAL_BASE_URL
    ...
    def _mock_chat(self, request: ChatCompletionRequest) -> ChatCompletionResponse:
        content = f"[Mistral mock] Response to: {request.messages[-1].content}"
    ...
    async def _mock_stream(self, request: ChatCompletionRequest) -> AsyncIterator[ChatCompletionChunk]:
        words = f"[Mistral mock stream] {request.messages[-1].content}".split()
        for word in words:
            yield ChatCompletionChunk(
                id=f"chatcmpl-{uuid.uuid4().hex}",
                created=int(time.time()),
