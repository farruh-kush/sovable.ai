...
Using `sed` (Linux) or manually editing the files, replace `latest` with your `$VERSION` tag in `infrastructure/k8s/microservices/gateway.yaml` and `infrastructure/k8s/microservices/microservices.yaml`. Also, ensure the image paths point to your `$REGISTRY`.
...
kubectl apply -f infrastructure/k8s/microservices/microservices.yaml

# Apply Gateway (Deployment, Service, and Ingress)
kubectl apply -f infrastructure/k8s/microservices/gateway.yaml
```
