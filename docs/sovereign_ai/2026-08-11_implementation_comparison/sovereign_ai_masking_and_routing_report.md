...
## 14. Implementation and procurement options

The platform can be delivered through four paths: a ready-to-buy product, a fully open-source assembly, open-source foundations with proprietary development, or a ground-up build. The recommended strategy is **buy speed, own the trust boundary, and build the sovereign differentiation**. Use a local or commercially supported product for a constrained pilot, then migrate the privacy, policy, routing, audit, RAG authorization, agent approval, and billing control plane to open-source foundations plus proprietary development.

| Path | Pilot one-time | Pilot monthly | Production one-time | Production monthly | Pilot time | Sovereignty fit |
| --- | ---: | ---: | ---: | ---: | --- | --- |
| Ready to buy | $10k–$75k | $2k–$15k | $50k–$250k | $8k–$35k | 1–3 months | Medium; high only with local/private deployment. |
| Fully open-source | $60k–$180k | $8k–$30k | $180k–$500k | $20k–$70k | 3–6 months | High, but operationally demanding. |
| Open-source + own development | $120k–$300k | $12k–$40k | $300k–$900k | $30k–$100k | 4–8 months | High and highly customizable. |
| Own from scratch | $350k–$650k | $20k–$60k | $750k–$2.0m | $50k–$180k | 9–15 months | Highest theoretical control, highest execution risk. |

These are planning estimates, not vendor quotes. They exclude major model training, national data-center construction, taxes/customs, formal certification fees, 24/7 SOC outsourcing, and high-volume model-token charges. A separate self-hosted GPU and domestic infrastructure envelope of approximately **$200k–$1.5m** may be required, depending on model size, redundancy, storage, networking, data-center readiness, and disaster recovery.

| Decision dimension | Ready to buy | Fully open-source | Open-source + own development | Own from scratch |
| --- | ---: | ---: | ---: | ---: |
...
| Uzbekistan-specific customization | 3/5 | 4/5 | 5/5 | 5/5 |
...
*Figure 3. Weighted strategy fit and one-time implementation cost midpoint. The score is a planning model; cost bars exclude variable model usage.*

### 14.1 Ready to buy

A commercial or local platform provides the fastest route to a pilot with provider adapters, dashboards, guardrails, retries, quotas, and support. Microsoft documents multi-provider AI gateway capabilities, token quotas, content safety, semantic caching, load balancing, and telemetry [16]. AWS publishes provider-side guardrail filters and charges per text unit or image, separately from model inference [23]. Portkey documents universal API routing, fallbacks, retries, circuit breakers, multimodality, custom hosts, budgets, and self-hosting [17]. UzCloud is a locally relevant candidate because its public page advertises Uzbek-market access to multiple models, masking of PINFL/passports/cards, audit, local document processing, and own-data-center deployment [14].

The disadvantages are vendor lock-in, recurring platform and provider charges, limited control over proprietary policy formats, and the need to verify where prompts, responses, logs, mappings, embeddings, images, and backups are processed. Ready-to-buy is appropriate for public, synthetic, or explicitly approved data and for a constrained pilot. It is not automatically suitable for Tier 1 government or bank-secret data.

### 14.2 Fully open-source

A fully open-source assembly can combine LiteLLM or Portkey Gateway, Envoy/Kong, Keycloak, Presidio, NeMo Guardrails or Guardrails AI, Llama Guard, vLLM/TGI, Qdrant/pgvector, MinIO/Ceph, Kubernetes, and OpenTelemetry [7] [8] [9] [10] [11] [12]. The principal benefit is complete deployment control and low core licensing cost. The principal cost is operational: the buyer owns integration, patching, provider connector changes, security hardening, incident response, model evaluation, and 24/7 reliability.

Fully open-source is a strong fit for a capable platform or research organization. It is not “free,” because GPUs, HSM/KMS, identity, SRE, support, audit, testing, and security operations remain paid requirements.

### 14.3 Open-source with proprietary development

This is the recommended strategic path. Reuse mature commodity components, but develop the modules that encode sovereign differentiation: Uzbek Latin/Cyrillic and Russian recognizers, policy and classification engine, encrypted mapping vault, tenant isolation, billing, routing policy, RAG authorization, agent/tool capability broker, human approval workflows, audit evidence, and local deployment controls. Keep provider adapters isolated and replaceable so upstream dependencies can be changed without changing client applications.

This path has the best balance of sovereignty, time to pilot, customization, and long-term independence. It requires a permanent engineering, security, and operations team, but it avoids the risk of reimplementing every mature platform primitive.

### 14.4 Own solution from scratch

A ground-up build offers maximum architectural independence, but it also requires the project to reproduce years of ecosystem work: provider quirks, streaming, retries, token normalization, rate limits, model metadata, image and audio paths, observability, and operational edge cases. It is slower, more expensive, and harder to prove reliable.

Use this path only after requirements and traffic patterns are validated or where a national utility requires control beyond what open-source foundations can provide. Even then, selectively reuse standards and infrastructure primitives instead of rewriting cryptography, Kubernetes, telemetry, databases, and inference runtimes.

### 14.5 Procurement recommendation by customer

| Customer | Recommended path |
| --- | --- |
| Ministry or agency needing a quick pilot | Ready-to-buy/local partner pilot, then migrate to OSS + proprietary control plane. |
| Central government or security-sensitive agency | OSS + proprietary development with domestic models, keys, audit, and restricted egress. |
| Commercial bank | OSS + proprietary development inside the bank perimeter, with optional commercial support. |
| SME or startup | Ready-to-buy local gateway or shared enterprise edition for low-risk data. |
| National AI platform operator | OSS + proprietary development as the baseline, with selective ground-up modules. |
| Research laboratory or university | Fully open-source for experimentation and local control. |

> **Final recommendation:** Ready-to-buy pilot → open-source foundation → proprietary sovereign control plane → selective ground-up components.
