...
* `microservices/auth/alembic.ini` and `microservices/billing/alembic.ini` have been added.
...
* Tests have been properly distributed into `microservices/<service>/tests/` (e.g., `microservices/gateway/tests/test_gateway.py`).
...
1. **Google & Mistral Adapters:** The routing configuration references `google` and `mistral` providers, but their adapter implementations (`google_adapter.py`, `mistral_adapter.py`) are not yet present in `microservices/provider/src/provider/adapters/`.
