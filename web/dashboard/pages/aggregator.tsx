...
export default function Aggregator() {
  ...
  return <NationalShell><main className="national-page"><Eyebrow>{t('agg.eyebrow')}</Eyebrow><h1>{t('agg.title')}</h1><p className="national-lede">{t('agg.lede')}</p>
    ...
    <section className="workflow-card"><Eyebrow>{t('agg.pipeline')}</Eyebrow><div className="workflow-steps">{['Request','Mask','Route','Restore','Response'].map((step, index) => <div className="workflow-step" key={step}><span>{`0${index + 1}`}</span><b>{step}</b><small>{['Protected input','Sensitive data removed','Policy decision','Safe output restore','Normalized response'][index]}</small>{index < 4 && <i>→</i>}</div>)}</div></section>
