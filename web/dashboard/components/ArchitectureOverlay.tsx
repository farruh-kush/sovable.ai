...
export default function ArchitectureOverlay() {
  ...
  const modelTop = models[0].y
  const modelBottom = models[models.length - 1].y
  const userTop = users[0].y
  const userBottom = users[users.length - 1].y

  return (
    <svg className="architecture-overlay" aria-hidden="true">
      <path d={`M ${modelBus} ${modelTop} V ${modelBottom}`} stroke={slate} {...wire} />
      ...
      <circle cx={modelBus} cy={modelTop} r={5} fill={slate} stroke="#fff" strokeWidth={2} />
      <circle cx={modelBus} cy={modelBottom} r={5} fill={slate} stroke="#fff" strokeWidth={2} />
      <path d={`M ${modelBus} ${coreLeft[0].y} H ${coreLeft[0].x}`} stroke={gold} {...wire} />
      <path d={`M ${modelBus} ${coreLeft[1].y} H ${coreLeft[1].x}`} stroke={gold} {...wire} />
      {coreLeft.map((point, index) => dot(point, gold, `core-left-${index}`))}
      ...
      <path d={`M ${platformRight[0].x} ${platformRight[0].y} H ${userBus} V ${userBottom}`} stroke={gold} {...wire} />
      {platformRight.slice(1).map((point, index) => (
        <path key={`platform-bus-${index}`} d={`M ${point.x} ${point.y} H ${userBus}`} stroke={gold} {...wire} />
      ))}
      {platformRight.map((point, index) => dot(point, gold, `platform-right-${index}`))}
      ...
      <circle cx={userBus} cy={userTop} r={5} fill={gold} stroke="#fff" strokeWidth={2} />
      <circle cx={userBus} cy={userBottom} r={5} fill={gold} stroke="#fff" strokeWidth={2} />
    </svg>
  )
}
