...
async function request(path: string, init: RequestInit = {}): Promise<AuthResponse> {
  const response = await fetch(`${authBase}${path}`, { ...init, headers: { 'Content-Type': 'application/json', ...(init.headers || {}) } })
  const payload = await response.json().catch(() => ({}))
  if (!response.ok) throw new Error(payload.detail || payload.message || `Authentication request failed (${response.status})`)
  return payload
}
...
export async function getCurrentUser() {
  ...
  const response = await fetch(`${authBase}/auth/me`, { headers: { Authorization: `Bearer ${token}` } })
...
export async function logout() {
  ...
  await request('/auth/logout', { method: 'POST', body: JSON.stringify({ refresh_token }) }).catch(() => undefined)
