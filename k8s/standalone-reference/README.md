...
This package deploys the synchronized five-service reference implementation: gateway, auth, router, provider, and billing. It is intentionally separate from the repository’s existing `k8s/` manifests so it can be evaluated and rolled out independently.
