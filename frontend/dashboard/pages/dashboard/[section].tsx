...
import { api, getModels, previewMasking } from '../../lib/api'
...
const pages: Record<string, { eyebrow: string; title: string; description: string; cards: [string, string, string][] }> = {
  ...
  billing: { eyebrow: 'BILLING · UZS', title: 'Budgets and payments in Uzbek so\'m', description: 'Manage plans, usage thresholds, invoices, and payments in Uzbek so\'m with Humo, Uzcard, and Mastercard support.', cards: [['Currency', 'Uzbek so\'m (UZS) ledger and invoices', 'Default'], ['Payment methods', 'HUMO · UZCARD · MASTERCARD', 'Supported'], ['Budget guardrails', 'Monthly cap and alert thresholds', 'Protected'], ['Payment provider', 'Acquirer adapter and signed callbacks', 'Credentials required'], ['Invoices', 'Usage exports and invoice history', 'Event-backed'], ['Refunds', 'Full and partial refund states', 'Governed']] },
...
function MaskingPreview() {
  const [text, setText] = useState('My email is user@example.com and my phone is +998 90 123 45 67.')
  const [result, setResult] = useState<any>(null)
  const [status, setStatus] = useState<'idle' | 'loading' | 'error'>('idle')
  async function submit(event: React.FormEvent) {
    event.preventDefault(); setStatus('loading')
    try { setResult(await previewMasking([{ role: 'user', content: text }])); setStatus('idle') }
    catch { setStatus('error') }
  }
  return <section className="card privacy-preview"><div className="table-title"><div><span className="eyebrow">LIVE POLICY CHECK</span><h3>Preview data masking</h3></div><span className="pill">Provider-safe</span></div><p className="muted">Sensitive values are transformed before provider routing. Original values stay request-local and are never returned by this preview.</p><form onSubmit={submit}><textarea className="privacy-input" value={text} onChange={event => setText(event.target.value)} aria-label="Text to mask" rows={4} /><button className="button primary" type="submit" disabled={status === 'loading'}>{status === 'loading' ? 'Checking…' : 'Preview masking'}</button></form>{status === 'error' && <div className="data-state error">Masking preview requires a valid API key and a healthy gateway.</div>}{result && <div className="privacy-output"><div><strong>{result.detected_count}</strong> sensitive value(s) detected</div><pre>{JSON.stringify(result.masked_messages, null, 2)}</pre><div className="privacy-meta">{(result.token_labels || []).join(' · ') || 'No sensitive values detected'}</div></div>}</section>}
...
export default function UserSection({ section }: { section: string }) {
  ...
  return <Layout>
    ...
    {section === 'privacy' ? <MaskingPreview /> : section === 'models' ? <section className="card table-card"><div className="table-title"><div><span className="eyebrow">LIVE CATALOG</span><h3>Available models</h3></div><span className="pill">{state === 'ready' ? `${models.length} returned` : state}</span></div>{state === 'error' ? <div className="data-state error">Model service unavailable. Retry when the gateway is healthy.</div> : state === 'loading' ? <div className="data-state">Loading governed catalog…</div> : models.length ? <div className="table-wrap"><table><thead><tr><th>Model</th><th>Provider</th><th>Context</th><th>Policy</th></tr></thead><tbody>{models.map((item: any) => <tr key={item.id || item.model}><td><strong>{item.id || item.model}</strong></td><td>{item.provider || 'Managed route'}</td><td>{item.context_length || '—'}</td><td><span className="status-label positive">Eligible</span></td></tr>)}</tbody></table></div> : <div className="data-state">No models are currently returned by the provider catalog.</div>}</section> : <section className="dashboard-grid"><article className="card"><span className="eyebrow">NEXT ACTION</span><h3>Connect this surface to governed events</h3><p className="muted">This screen is ready for the control-plane API contract. It intentionally avoids fabricated metrics until the corresponding event stream is available.</p><Link href="/documentation" className="button secondary">Review contract</Link></article><article className="card"><span className="eyebrow">CONTROL STATUS</span><h3>Policy boundary active</h3><p className="muted">All provider calls remain behind the gateway, router, privacy policy, and billing controls.</p><span className="status-label positive">Protected path</span></article></section>}
  </Layout>
}
