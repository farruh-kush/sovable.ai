...
def _send_directmail(settings: AuthSettings, recipient: str, subject: str, text: str, html: str) -> None:
    if not settings.directmail_access_key_id or not settings.directmail_access_key_secret or not settings.directmail_account_name:
        raise RuntimeError("Alibaba DirectMail is not configured")
...
async def send_activation_email(settings: AuthSettings, recipient: str, activation_url: str) -> None:
    subject = "Activate your Solvable AI account"
    text = (
        "Activate your Solvable AI account by opening this link:\n\n"
        f"{activation_url}\n\n"
        f"This link expires in {settings.activation_link_ttl_seconds // 60} minutes and can be used once."
    )
    html = (
        ...
        f"<p>This link expires in {settings.activation_link_ttl_seconds // 60} minutes and can be used once.</p>"
    )
