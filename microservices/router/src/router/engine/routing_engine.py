...
class RoutingEngine:
        ...
        for provider_name in candidates:
            try:
                ...
                response = await self._call_provider_chat(
                    provider_name, request, generation_id=generation_id
                )
    ...
    async def _resolve_candidates(
        ...
        # Step 1: Client-supplied explicit order or the configured policy.
        policy = self._get_routing_policy(request.model)
        if prefs and prefs.order:
            candidates = list(prefs.order)
        else:
            candidates = self._get_static_candidates(request.model)

        # Provider names are policy-controlled; an unknown name must never be
        # forwarded to the adapter service.
        known_providers = set(self._config.get("providers", {}))
        candidates = list(dict.fromkeys(p for p in candidates if p in known_providers))
        if not candidates:
            raise NoProvidersAvailableError(
                f"No configured provider is eligible for model '{request.model}'.",
        ...
        if not (prefs and prefs.order):
            ...
            if sort_by is None:
                strategy = policy.get("strategy") if policy else None
                sort_by = {"cost": "price", "price": "price", "latency": "latency"}.get(
                    strategy
                )
    ...
    def _get_routing_policy(self, model: str) -> dict:
        """Return a copy of the configured policy for a model or alias."""
        policy = self._config.get("routing", {}).get(model, {})
        return policy if isinstance(policy, dict) else {}

    def _get_static_candidates(self, model: str) -> List[str]:
        """Return the ordered provider candidates from routing.yaml.

        Dynamic aliases such as ``cheapest`` and ``fastest`` provide candidate
        providers and a strategy. A regular model may also rely on its model
        registry entry when no explicit fallback policy is present. Unknown
        models fail closed instead of being forwarded to every adapter.
        """
        routing = self._get_routing_policy(model)
        if routing:
            primary = routing.get("primary")
            fallbacks = routing.get("fallback", [])
            configured_candidates = routing.get("candidates", [])
            if primary:
                return [primary, *fallbacks]
            return list(configured_candidates or fallbacks)

        model_config = self._config.get("models", {}).get(model, {})
    ...
    def _filter_by_data_policy(self, candidates: List[str]) -> List[str]:
        ...
        return [
            ...
            and providers_config.get(p, {}).get("data_policy", {}).get("zero_data_retention", False)
        ]
    ...
    async def _call_provider_chat(
        ...
        async with httpx.AsyncClient(
            ...
            response = await client.post(
                "/adapt/chat/completions", json=payload, headers=headers
            )
